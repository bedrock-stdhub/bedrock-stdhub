...
export default async function init() {
  ...
  // Deprecated. The package `prismarine-nbt` does not work with ESBuild -> pkg.
  /*
  ...
  */
  ...
  const originBytes = LevelDatBytes.subarray(startOffset, endOffset);
  if (!originBytes.equals(patchedExpBytes)){
    const frontBytes = LevelDatBytes.subarray(8, startOffset);
    const backBytes = LevelDatBytes.subarray(endOffset);
